...
use crate::{
    prelude::*,
    rendering::{RenderedInventory, RenderedSlot},
};
...
impl Plugin for InventoryNodePlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            Update,
            (
                slot_node::update_image_cell_scale,
                item_node::update_item_node_image,
                style_inventory_node,
            ),
        );
    }
}
...
fn spawn_inventory_node(
    ...
    for (entity, target, back_ground, mut node, pickable) in new {
        ...
        let Some(inventory) = inventory_manager.read_inventory(target) else {
            warn!(
                "Failed to get Inventory({:?}) for Node({:?})",
                target.inventory, entity
            );
            continue;
        };
...
fn update_inventory_node(
    ...
    mut inventory_nodes: Query<
        (
            Ref<RenderedInventory>,
            &mut Node,
            Option<&ItemNodes>,
            Option<&Pickable>,
        ),
    ...
    for message in changes.read() {
        ...
        let Some(inventory) = assets.get(*id) else {
            warn!(
                "Received modified event for Inventory asset {:?} that is not currently loaded",
                id
            );
            continue;
        };
        for rendering in inventory.windows() {
            let Ok((root, mut node, item_nodes, picicking)) = inventory_nodes.get_mut(rendering)
            else {
                // TODO dont warn as this is just the case when sprite and node are both used
                warn!(
                    "Inventory asset {:?} modified but failed to find RenderedInventory component for entity {:?}",
                    id, rendering
                );
                continue;
            };
            ...
            for slot_entity in root.iter() {
                let Ok(slot) = slots.get(slot_entity) else {
                    // TODO dont warn as this is just the case when sprite and node are both used
                    warn!(
                        "Failed to get SlotNode component for entity {:?} while updating inventory node for Inventory asset {:?}",
                        slot_entity, id
                    );
            ...
            for slot in remove {
                warn!(
                    "Despawning slot entity {:?} for removed slot in Inventory asset {:?}",
                    slot, id
                );
            ...
            for new in need {
                let shape = inventory
                    .get_slot(new)
                    .expect("We just checked and it existed");
                let size = shape.layout.size();
            ...
            if let Some(item_nodes) = item_nodes {
                ...
                for item in item_nodes.iter() {
                    ...
                    let Ok(target) = render_nodes.get(item) else {
                        trace!(
                            "Failed to get RenderedItem component for entity {:?} while updating inventory node for Inventory asset {:?}",
                            item, id
                        );
                        continue;
                    };
                    if items.remove(&target.item) {
                        commands
                            .entity(item)
                            .insert(RenderedItem { item: target.item });
                    } else {
                        // remove already existing items from the set
                        trace!(
                            "Despawning ItemNode entity {:?} for removed item in Inventory asset {:?}",
                            item, id
                        );
            ...
            for new in items {
                let shape = inventory
                    .get_shape(new)
                    .expect("New items should all definityl be in inventory");
...
fn style_inventory_node(
    ...
    changed: Populated<
        (Entity, &mut Node),
        Or<(
            AssetChanged<InventoryStyleHandle>,
            Changed<InventoryStyleHandle>,
        )>,
    >,
) {
